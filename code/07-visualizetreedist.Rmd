...
# recall # of tree distance will be around ~30 less than total number of SPIDs due to some SPIDs having less than 2 taxa covered 
```
...
#### identifying outliers of branch score difference

```{r}
#calculates z score for all branch score difs
z <- scale(treedist$branch_score_difference)

# filters for just outliter z scores (z > 3) 
...
#### scatter plot of BSD for trees with full taxa coverage (18 taxa hist and scatter)
...
# filter mosttaxa to pull just SPIDs that have z > 3 in the z matirx 
z_most <- scale(mosttaxa$branch_score_difference)
outliers18_z <- mosttaxa[abs(z_most) > 3, c("SPID", "branch_score_difference")]

write.csv(mosttaxa$SPID, "../output/treedist/fullcoverageSPID.txt", row.names = F, quote = F)
write.csv(outliers18_z$SPID, "../output/treedist/fullcoverageoutlierSPID.txt", row.names = F, quote = F)
...
### [**how many unique genes are there to each group?**]{.underline}
...
exclusive_genes_count <- data.frame(
...
exclusive_genes_count

```

```{r}
exclusive_genes <- function(group, presence_mat) {
  others <- setdiff(colnames(presence_mat), group)

  keep <- rowSums(presence_mat[, group, drop = FALSE]) == length(group) &
          rowSums(presence_mat[, others, drop = FALSE]) == 0

  rownames(presence_mat)[keep]
}

# Get vectors of genes for each group
groups_list <- list(
  oysters_only    = exclusive_genes(oysters, presence_df),
  scallops_only   = exclusive_genes(scallops, presence_df),
  mussels_sw_only = exclusive_genes(mussels_sw, presence_df),
  clams_only      = exclusive_genes(clams, presence_df),
  mussel_fw_only  = exclusive_genes(mussel_fw, presence_df)
)

# Determine max number of rows
max_len <- max(sapply(groups_list, length))

# Pad with NA so all columns have same length
groups_padded <- lapply(groups_list, function(vec) {
  length(vec) <- max_len   # extends with NAs automatically
  vec
})

# Build final data frame
exclusive_genes_listed <- as.data.frame(groups_padded, stringsAsFactors = FALSE)

exclusive_genes_listed
```

## Life History Groups

### [**how many unique genes are there to each life history / sexual system?**]{.underline}

create our groups

```{r}

gonochoristic <- c("GCF_026914265.1", "GCF_020536995.1", "GCF_036588685.1", "GCF_026571515.1" , "GCF_002113885.1" , "GCF_021869535.1" , "GCF_033153115.1" , "GCF_031769215.1")

gonochroistic_moslty <- c("GCF_036588685.1" , "GCF_026571515.1" , "GCF_002113885.1" , "GCF_021869535.1")

gonochoristic_always <- c("GCF_026914265.1" ,"GCF_020536995.1" , "GCF_033153115.1" , "GCF_031769215.1")

hermaphroditic <- c("GCF_002022765.2" , "GCF_963853765.1" , "GCF_041381155.1" ,"GCF_902652985.1" , "GCF_025612915.1" , "GCF_947568905.1" , "GCF_021730395.1" , "GCF_032062105.1")

sim_hermaphroditic <- c("GCF_041381155.1" , "GCF_902652985.1")
  
seq_hermaphroditic <- c("GCF_002022765.2", "GCF_963853765.1", "GCF_025612915.1", "GCF_947568905.1", "GCF_021730395.1", "GCF_032062105.1")
...
exclusive_genes_count <- data.frame(
  gonochoristic_only = exclusive_count(gonochoristic, presence_df),
  gonochroistic_moslty_only = exclusive_count(gonochroistic_moslty, presence_df),
  gonochoristic_always_only = exclusive_count(gonochoristic_always, presence_df),
  hermaphroditic_only = exclusive_count(hermaphroditic, presence_df),
  sim_hermaphroditic_only = exclusive_count(sim_hermaphroditic, presence_df),
  seq_hermaphroditic_only = exclusive_count(seq_hermaphroditic, presence_df)
)
...
# Create vector of taxa groups, matching the original columns of presence_df
taxa_groups <- rep(NA, ncol(presence_df))
names(taxa_groups) <- colnames(presence_df)

taxa_groups[gonochoristic] <- "gonochoristic"
taxa_groups[sim_hermaphroditic] <- "simultaneous hermaphrodite"
taxa_groups[seq_hermaphroditic] <- "sequential hermaphrodite"

# Annotation now needs to match the NEW "rows" (after transposition)
annotation_row <- data.frame(Group = taxa_groups)
rownames(annotation_row) <- names(taxa_groups)

# Filter genes with less than 18 taxa present
presence_df_filtered <- presence_df[rowSums(presence_df) < 18, ]

# Swap rows & columns by transposing
pheatmap(
  t(presence_df_filtered),
  cluster_rows = TRUE,
  cluster_cols = TRUE,
  annotation_row = annotation_row,
  show_rownames = TRUE,     # taxa names
  show_colnames = FALSE,    # gene names (too many to show)
  color = c("white", "blue"),
  legend = TRUE,
...
```{r}
# 7. Plot heatmap
png("../output/treedist/gene_presence_heatmap.png", width = 800, height = 600)
...
pheatmap(
  ...
  color = c("white", "blue"),
  legend = TRUE,
  main = "Gene presence/absence (taxa ordered by tree)"
)
