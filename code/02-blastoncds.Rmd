...
-in ../data/genomes_refseq/ncbi_dataset/data/GCF_041381155.1/cds_from_genomic.fna \
-dbtype nucl \
-out ../blastdb/GCF_041381155.1/GCF_041381155.1/
```

preview file and check \# of sequences

```{bash}
head ../data/genomes_refseq/ncbi_dataset/data/GCF_041381155.1/cds_from_genomic.fna
echo "Number of Sequences"
grep ">" -c ../data/genomes_refseq/ncbi_dataset/data/GCF_041381155.1/cds_from_genomic.fna
```

```{bash}
echo "Total length in Gbp"
grep -v ">" ../data/genomes_refseq/ncbi_dataset/data/GCF_041381155.1/cds_from_genomic.fna | \
tr -d '\n' | wc -c | awk '{printf "%.3f\n", $1/1e9}'
```
...
```{bash}
# fasta= "../data/genomes_refseq/ncbi_dataset/data/GCF_041381155.1/cds_from_genomic.fna"
# output="../output/blastout/GCF_041381155.1.tab"

/home/shared/ncbi-blast-2.15.0+/bin/tblastn \
-query ../data/SwissProt-GO:0007276.fa \
-db ../blastdb/GCF_041381155.1/GCF_041381155.1 \
-out ../output/blastout/GCF_041381155.1.tab \
-evalue 1E-10 \
-num_threads 20 \
-max_target_seqs 1 \
-max_hsps 1 \
...
```{bash}
head ../output/blastout/GCF_041381155.1.tab
```

```{bash}
wc -l ../output/blastout/GCF_041381155.1.tab
```
...
echo -e "qseqid\tsseqid\tpident\tlength\tmismatch\tgapopen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore" > ../output/blastout/GCF_041381155.1_with_headers.tab
cat ../output/blastout/GCF_041381155.1.tab >> ../output/blastout/GCF_041381155.1_with_headers.tab

head -2 ../output/blastout/GCF_041381155.1_with_headers.tab

```

```{bash}

tr '|' '\t' < ../output/blastout/GCF_041381155.1_with_headers.tab \
> ../output/blastout/GCF_041381155.1_sep.tab

head -2 ../output/blastout/GCF_041381155.1_sep.tab
```
...
blastout <- read.delim("../output/blastout/GCF_041381155.1_with_headers.tab")
head(blastout)
```
...
tr '|' '\t' < ../output/blastout/GCF_041381155.1.tab \
> ../output/blastout/GCF_041381155.1_sep.tab

head -3 ../output/blastout/GCF_041381155.1_sep.tab
```

```{r}

SPID <- read.csv("../output/blastout/GCF_041381155.1_sep.tab", sep = "", header = F)
head(SPID)

write.csv(SPID$V2, "../output/GCF_041381155.1_SPID.txt", row.names = F, col.names = F, quote = F)
```
...
annot <- read.csv("../output/blastout/GCF_041381155.1_with_headers.tab", sep = "")
head(annot)
...
uniprot <- read.delim("../output/GCF_041381155.1_uniprotout")
head(uniprot)
...
write.csv(annot_w_GO, "../output/GCF_041381155.1_annotation.tab", sep = "")

temp <- read.csv("../output/GCF_041381155.1_annotation.tab")
```
...
```{r}
write.csv(gene_list, "../output/genelist_GCF_041381155.1.csv")
```
