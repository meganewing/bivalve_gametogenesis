...
-in ../data/genomes_refseq/ncbi_dataset/data/GCF_000327385.1/cds_from_genomic.fna \
-dbtype nucl \
-out ../blastdb/GCF_000327385.1/GCF_000327385.1/
```

preview file and check \# of sequences

```{bash}
head ../data/genomes_refseq/ncbi_dataset/data/GCF_000327385.1/cds_from_genomic.fna
echo "Number of Sequences"
grep ">" -c ../data/genomes_refseq/ncbi_dataset/data/GCF_000327385.1/cds_from_genomic.fna
```

```{bash}
echo "Total length in Gbp"
grep -v ">" ../data/genomes_refseq/ncbi_dataset/data/GCF_000327385.1/cds_from_genomic.fna | \
tr -d '\n' | wc -c | awk '{printf "%.3f\n", $1/1e9}'
```
...
```{bash}
# fasta= "../data/genomes_refseq/ncbi_dataset/data/GCF_000327385.1/cds_from_genomic.fna"
# output="../output/blastout/GCF_000327385.1.tab"

/home/shared/ncbi-blast-2.15.0+/bin/tblastn \
-query ../data/SwissProt-GO:0007276.fa \
-db ../blastdb/GCF_000327385.1/GCF_000327385.1 \
-out ../output/blastout/GCF_000327385.1.tab \
-evalue 1E-10 \
-num_threads 20 \
-max_target_seqs 1 \
-max_hsps 1 \
...
```{bash}
head ../output/blastout/GCF_000327385.1.tab
```

```{bash}
wc -l ../output/blastout/GCF_000327385.1.tab
```
...
echo -e "qseqid\tsseqid\tpident\tlength\tmismatch\tgapopen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore" > ../output/blastout/GCF_000327385.1_with_headers.tab
cat ../output/blastout/GCF_000327385.1.tab >> ../output/blastout/GCF_000327385.1_with_headers.tab

head -2 ../output/blastout/GCF_000327385.1_with_headers.tab

```

```{bash}

tr '|' '\t' < ../output/blastout/GCF_000327385.1_with_headers.tab \
> ../output/blastout/GCF_000327385.1_sep.tab

head -2 ../output/blastout/GCF_000327385.1_sep.tab
```
...
blastout <- read.delim("../output/blastout/GCF_000327385.1_with_headers.tab")
head(blastout)
```
...
tr '|' '\t' < ../output/blastout/GCF_000327385.1.tab \
> ../output/blastout/GCF_000327385.1_sep.tab

head -3 ../output/blastout/GCF_000327385.1_sep.tab
```

```{r}

SPID <- read.csv("../output/blastout/GCF_000327385.1_sep.tab", sep = "", header = F)
head(SPID)

write.csv(SPID$V2, "../output/genelist/GCF_000327385.1_SPID.txt", row.names = F, col.names = F, quote = F)
```

### GO terms were gathered from [uniport ID mapping](https://www.uniprot.org/id-mapping), with the text file generated above being used as the input. Source database: UniProtKB AC/ID. Target database: UniProtKB. columns were customized to include biological proccess GO information, GO ID, and GO (basically everything under the GO tab except cellular component and molecular processes). output was saved as a .tsv.
...
annot <- read.csv("../output/blastout/GCF_000327385.1_with_headers.tab", sep = "")
head(annot)
...
uniprot <- read.delim("../output/genelist/GCF_000327385.1_uniprotout")
head(uniprot)
...
write.csv(annot_w_GO, "../output/genelist/GCF_000327385.1_annotation.tab", sep = "")

temp <- read.csv("../output/genelist/GCF_000327385.1_annotation.tab")
```
...
```{r}
write.csv(gene_list, "../output/genelist/genelist_GCF_000327385.1.csv")
```
