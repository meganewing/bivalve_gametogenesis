...
tr '|' '\t' < ../output/blastout/GCF_963853765.1.tab \
> ../output/blastout/GCF_963853765.1_sep.tab

head -3 ../output/blastout/GCF_963853765.1_sep.tab
```

```{r}

SPID <- read.csv("../output/blastout/GCF_963853765.1_sep.tab", sep = "", header = F)
head(SPID)

write.csv(SPID$V2, "../output/GCF_963853765.1_SPID.txt", row.names = F, col.names = F, quote = F)
```
...
annot <- read.csv("../output/blastout/GCF_963853765.1_with_headers.tab", sep = "")
head(annot)
...
uniprot <- read.delim("../output/GCF_963853765.1_uniprotout")
head(uniprot)
...
write.csv(annot_w_GO, "../output/GCF_963853765.1_annotation.tab", sep = "")
```

# GENE LIST

multiple results per seq. id. want to keep the one with the highest bitscore
...
gene_list <- data.frame(annot_w_GO_top$qseqid, annot_w_GO_top$sseqid, annot_w_GO_top$evalue, annot_w_GO_top$bitscore, annot_w_GO_top$SPID, annot_w_GO_top$Entry.Name, annot_w_GO_top$Protein.names, annot_w_GO_top$Gene.Names, annot_w_GO_top$Organism, annot_w_GO_top$Length, annot_w_GO_top$Gene.Ontology.IDs, annot_w_GO_top$Gene.Ontology..biological.process., annot_w_GO_top$Gene.Ontology..GO., annot_w_GO_top$Developmental.stage, annot_w_GO_top$Induction, annot_w_GO_top$Induction, annot_w_GO_top$Tissue.specificity)
...
write.csv(gene_list, "../output/genelist_GCF_963853765.1.csv")
```
