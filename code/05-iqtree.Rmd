...
run_iqtree() {
    ...
    ### NEW: detect full outgroup header & build rename map if present
    for aligned_fasta in /path/to/fastas/*.fasta; do
      outgroup=$(grep "^>GCF_000327385\.1" "$aligned_fasta" | sed 's/^>//')
      echo "Outgroup for $aligned_fasta is: $outgroup"
    # you can then pass $outgroup to iqtree -o "$outgroup"
    done
